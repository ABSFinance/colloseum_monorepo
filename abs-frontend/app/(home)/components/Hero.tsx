    ...
    <section className="relative pt-24 pb-16 md:pt-32 md:pb-24 overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          >
            ABS Finance uses AI models to find and allocate your assets to the
            highest-yielding opportunities across solana and all the vaults.
          </motion.p>
          ...
          >
            <Link href={"/vaults"}>
              <Button
                size="lg"
                className="bg-white text-black hover:bg-gray-200 cursor-pointer transition-colors text-base"
              >
                Get Started
      ...
      <div className="absolute inset-0 overflow-hidden z-0 opacity-20">
        ...
        {Array.from({ length: 10 }).map((_, i) => (
          <div
            key={i}
            className="absolute top-0 left-0 right-0 h-px bg-gray-800"
            style={{ top: `${i * 10}%` }}
          />
        ))}
        {Array.from({ length: 10 }).map((_, i) => (
          <div
            key={i}
            className="absolute top-0 bottom-0 left-0 w-px bg-gray-800"
            style={{ left: `${i * 10}%` }}
          />
        ))}
      </div>
